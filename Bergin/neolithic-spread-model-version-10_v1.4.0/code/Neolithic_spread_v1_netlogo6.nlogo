...
to set-shading
  ...
  carefully [
    ...
    set pcolor scale-color 55 ecol-index 10 1] [ ;; scale color of suitable from dark (most suitable) to light
...
to IDD[threshold]
  ...
  set spreadzone spreadzone with [not isNaN ecol-index];; make sure a patch is not water
  ...
  if any? spreadzone with [adjusted-index >= threshold] [ ;; only pick patches with sufficiently good environment
    ...
    set spreadzone spreadzone with [adjusted-index = best]
    ask one-of spreadzone [
