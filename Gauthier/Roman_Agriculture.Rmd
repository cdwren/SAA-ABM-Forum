...
calc_utility <- function(farming_labor, runoff = 1, p = 1, b = max_yield, maintainance_labor = 1 - farming_labor, 
                         j = 0.2, r = 1, k = 0.6, a = 1, w = 0){
  le <- 1 - farming_labor - maintainance_labor
  q <- infrastructure_performance(maintainance_labor) * runoff
  p * b * farming_labor ^ j * (q + r) ^ k * a ^ (1 - j - k) + w * w
}

opts <- tibble(b = max_yield, runoff = .3, farming_labor = c(.62,.62,1,1), r = 0:3, a = 30) %>%
  mutate(utility = calc_utility(b = b, runoff = runoff, farming_labor = farming_labor, r = r, a = a))
expand.grid(b = max_yield, runoff = .3, farming_labor = seq(0,1,.01), r = 0:3, a = 30) %>%
  mutate(utility = calc_utility(b = b, runoff = runoff, farming_labor = farming_labor, r = r, a = a)) %>%
  ggplot(aes(farming_labor, utility, color = r, group = r)) +
  geom_line(size = 2) +
